 ...
 vWorld = vLocal + uCentre;
        {
          ...
          float green = clamp(pow(700.0 / max(depth, 30.0), 0.25), 1.0, 2.20)
            * smoothstep(6.0, 34.0, depth);
          // Water cannot hold a wave taller than itself — see seaSwell.ts.
          float breakLimit = 0.3900 * max(depth, 0.0) / 22.0000;
          float softSum = pow(green, 4.0) + pow(breakLimit, 4.0);
          float shoal = softSum > 0.0
            ? green * breakLimit * pow(softSum, -0.2500)
            : 0.0;
          float swFade = shoal
            * (1.0 - smoothstep(6000.0, 7800.0, length(position.xz)));
          ...
          // trading places.
          float lift = max(sw * swFade, -max(0.0, depth - 4.0));
          transformed.y += lift;
...
#include <map_fragment>
        ...
        if (uLodMicroForce >= 0.0) micro = uLodMicroForce;
          ...
          diffuseColor.a *= 1.0 - smoothstep(6800.0, 8200.0, vSheet);

          if (diffuseColor.a < 0.01) discard;
        }
#include <normal_fragment_maps>
        {
          normal = normalize(normal + vec3(gRn.x, 0.0, gRn.y) * 0.75 * gBody + vec3(-vSwell.x, 0.0, -vSwell.y) * 1.5);
        }
