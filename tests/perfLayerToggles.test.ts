...
describe('LayerToggles', () => {
  ...
  it('this build has terrain, ocean, freshwater, weather, vegetation, resources and the three species; the unbuilt rows cannot be switched on', () => {
    // Phase 2 made `terrain` real, Phase 3 `ocean`, Phase 4 `freshwater`,
    // Phase 6 `vegetation`, Phase 5 `weather` (the sky: an HDRI dome,
    // the sun, the fog and the rain, driven by Kauaʻi's real weather) and
    // Phase 7 — the ecology pass — `resources` (what the island offers
    // to eat and drink) and `worms`, `aphids` and `flies`, one row per
    // species so each can be switched off and costed alone. Every other
    // layer (the player) is still a row that reads "not built" and
    // refuses to come on, which is §2.9 — an unavailable action must
    // never look functional. Each name arrives in the commit that makes
    // ITS toggle stop something, and not before.
    expect(BUILT_LAYERS).toEqual(['terrain', 'ocean', 'freshwater', 'weather', 'vegetation', 'resources', 'worms', 'aphids', 'flies']);
