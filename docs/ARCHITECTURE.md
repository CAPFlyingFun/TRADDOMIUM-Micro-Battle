...
src/
  ...
  creatures/    the island's animals, as CORE: species as data
                ...
                nothing.
                The Creature Lab (6.10) adds three files and a medium:
                demand.ts — the AI's target and word turned into the ONE
                movement Intent (`demandOf`) and the ONE integrator that
                moves a body by an Intent (`applyDemand`), which
                locomotion.ts's walk/burrow/fly/move now are; control.ts —
                the ControlLedger, control authority as plain
                {creature, controller} records kept by the session and
                never on the animal, so possession changes authority
                only and a server can own the book; labWorld.ts — a
                one-metre CreatureWorld (a block, a bumpy patch, a real
                puddle WaterQuery, five plants, the resource sites, the
                deterministic five spawns) built from the production
                queries, not a second engine. species.ts gains the
                `ground` medium and the two fire ants (queen, worker),
                placed by the Lab and generated by no cell; state.ts the
                words `defend` and `attack`.
                Creature Lab D (surfaces) adds surface.ts: a body's
                frame is a heading AND an up (`CreatureState.up`, the
                unit normal of what it stands on; `aheadOn` is the ONE
                rule for what a heading means on a face, the identity
                on the ground), the solids are axis-aligned boxes
                (`CreatureWorld.climbables`; the Lab's block is a slab
                on a pedestal so it has an underneath), an edge carries
                the frame by one rotation (`rotateBetween`), and
                `surfaceStep` keeps a climber's feet on whatever it is
                on. `CreatureSpecies.climber` says who: the insects,
                not the worm. The island has no solids and runs as
                before to the bit.
  ...
  control/      the PLAYER as a producer of the creatures' Intent, and
                ...
                Creature Lab D: `demandFromLook` reads the stick on the
                body's SURFACE (the look projected onto its face, right
                is ahead × up) and the follow camera orbits off the
                body's up, so a climbing ant's camera is out from the
                wall; the flat case is `demandFrom` to the bit.
  lab/          the Creature Lab, a dev tool (ARCHITECTURE §8): the 1 m
                ...
                in 6.10.
                `lab/stressTest.ts` is the STRESS TEST's arithmetic and
                is PURE — no three, no DOM, no clock of its own: a
                rolling frame rate over TIME (not over a frame count,
                which would silently lengthen exactly as the phone got
                slower), a settle that throws the rebuild frame away, a
                warm-up that fills the window before any reading is
                used, a seeded species draw so RUN AGAIN is the same
                test, the four thresholds recorded once each, and the
                copyable report. It is handed a RAW frame time and
                answers which species to spawn; the scene turns that
                into a body. The raised caps it needs come through the
                sanctioned `CreatureSimOptions.species` door as
                `creatures/labWorld.ts`'s `LAB_SPECIES_TABLE` — the
                island's numbers in every field but the cap — never a
                second engine, and `fauna/FaunaView` lends a rig per
                creature through `setPoolSize`, so the report can name
                which question was asked. Added in 6.11.
...
`creatures → input(Intent.ts)` and `creatures → actor(PlayerId)` were
added on 2026-09-09, with the Creature Lab's contracts, for the reason
the net arrow below was: an animal's brain and a possessing player's
thumbs must speak the ONE movement shape (`creatures/demand.ts` is the
third producer/consumer pair of `Intent`), and the ledger that says who
drives whom (`creatures/control.ts`) names a player by the one branded
id the session already owns. Both are pure and core; the core rule is
untouched.
...
| **6.10 Creature Lab (in progress)** | **One creature, two producers of one Intent, one integrator (Joshua's brief, 2026-09-09, §3/§42).** The contract, landed first and serially: `input/Intent.ts` gains `vertical`, `primary`, `secondary` (optional, absence is no request); `creatures/demand.ts` turns the AI's target and word into that Intent (`demandOf`) and is the ONE place a body is moved by one (`applyDemand`) — `locomotion.ts`'s `move` is now `applyDemand(demandOf())`, held to the old legs to 1e-6 over ten minutes of each medium; control is a LEDGER (`creatures/control.ts`: `{creature, controller: PlayerId}` records the session keeps, never a flag on the animal, so possession changes authority only and a server can own the book; an empty ledger is observer mode); the ants are a fourth MEDIUM, `ground` (walks the ground; the queen's `flight` spec adds the air words), with `defend`/`attack` as the ground's words, a `defensive` temperament, a per-species `paceExponent` (worm 1, ant 0.75 — Hurlbert 2008) and two placed, never generated, species (QUEEN, WORKER; rigs unmeasured and refused by the validator until the fauna pass measures them); the Lab is a dev tool over a one-metre `CreatureWorld` (`creatures/labWorld.ts`: block, bumpy patch, real puddle water, five plants, the resource sites, five deterministic spawns) using the production modules, never a second engine. Leaves in parallel: the possession/demand seam in the simulation, the ant rigs, the follow camera, the Lab scene, the queen's flight. **Creature Lab D (alpha.40, 2026-09-09) — surfaces, from Joshua's first device pass.** Three things from the phone: the stick's left and right were mirrored (the integrator called local +X the right; every rig's +X is its LEFT — `actor/Transform.ts`, `demand.planeStep`, `input/Intent.ts` now say right = ahead × up); the aphid's legs ran in slow motion (one fixed 0.4-body stride at 0.57 body lengths/s — `fauna/gait.strideLengthsAt` makes the stride the speed's, Mendes 2013 / Zollikofer 1994, the ants unchanged at their pace); and the insects had to climb (`creatures/surface.ts`: a body's frame is a heading AND an `up`, `aheadOn` the one rule for a heading on a face, `rotateBetween` carries the frame round an edge, `surfaceStep` keeps a climber on the pillar, the underside and the slab; `CreatureSpecies.climber`; the Lab's block a slab on a pedestal so it has an underneath, and the floor mesh no longer carries it — the z-fighting). `control/PlayerDemand.demandFromLook` reads the stick on the body's face with the SCREEN's right where the face has one; `FollowCamera` orbits off the body's up with a residual rotation eased over an edge; `fauna/` builds the root from (up, heading) with the up eased and the heading never. The island has no solids and runs to the bit as before; the identity soak holds. `probe:lab` drives the queen up the pillar, under the slab and off it (shots 9 and 10), and takes the aphid's model shot with the overlay off | nothing |
| **6.11 Creature Lab: the stress test (alpha.41)** | **How many living insects the one-metre room holds, measured rather than guessed (Joshua, 2026-09-10: "the main number I care about is how many fully active insects that 1 m room can sustain at 30+ FPS. The 10 FPS point is just the absolute breaking point").** STRESS on the bench spawns one seeded random species a second into the existing room through the normal systems — the same `CreatureSim` tick, senses, intent and locomotion, the same `FaunaView` — and records the count the five-second rolling average first falls through 45 / 30 / 20 / 10 fps, stops when it has held under 10 for five seconds, lets the crowd live another ten, and prints a copyable report with RUN AGAIN / RESET. `lab/stressTest.ts` is the arithmetic and is pure (no three, no DOM, no clock): the whole run plays out under vitest at any frame rate a phone might produce, which is the only way to test a thing whose subject is frame rate. Three seams made it honest rather than a second engine: the caps come through `CreatureSimOptions.species` as `LAB_SPECIES_TABLE` (the island's numbers in every field but the cap, `speciesProblems`-clean), so a run measures the phone and not `population.caps`; `FaunaView.setPoolSize` grows the rig pool one body at a time, so RIGS: ALL means an animated skeleton apiece and RIGS: RUNG means the detail ladder's budget, and the report NAMES which was asked; and the average is frames ÷ wall-clock over five SECONDS, never over a frame count, which would lengthen exactly as the phone slowed. Two honesty guards, both from a headless run that recorded every threshold at zero: a one-second SETTLE throws the rebuild frame away before the window opens (900 ms inside the first window reads as 51 fps on a 60 fps phone and would blame the crowd for the reset button), and a threshold crossed at zero creatures prints a CAVEAT over the headline saying the run measured an empty room. The report also says plainly what it does NOT measure: creatures do not collide with or avoid one another, because no such system exists yet. No probe — Joshua's phone is the instrument ("a probe won't work for this"); SwiftShader renders the bench at 2.7 fps | nothing |
| **6.12 Creature Lab: the census and the species runs (alpha.42)** | **Baseline A, measured on Joshua's phone 2026-09-10 and recorded in `docs/PERFORMANCE.md`: 52 full-rig creatures at 30+ fps, 178 before it broke.** The only trustworthy slope in it (20→10 fps, 96 creatures over 97 s) gives 0.52 ms of frame time per full-rig creature on a ~13 ms bench — which puts 230 full rigs at 133 ms, or 7.5 fps, so the multiplayer target cannot be met with a skeleton apiece and the next question is what a cheaper drawing buys. This release makes that question answerable. The report gains a DRAWN census — rigs against impostors at every threshold AND at the end, because 178 bodies carrying 178 skeletons and 178 carrying thirteen are not the same measurement and the totals alone cannot tell them apart — and a WHERE THE FRAME WENT split. That split has TWO KINDS OF NUMBER in it and says so: `drawMs` is the scene's own wall time around `FaunaView.update`, so its mean comes out of the frame and `everything else` is the remainder; `aiMs` is `CreatureSim.cost()`, which answers the LAST TICK's reading and the sim does not necessarily tick every frame, so it is printed OUTSIDE the split as a shape rather than a budget. A run driven with no samples leaves every census null and prints neither block — a zero standing in for "nobody counted" is the one error a pasted report never recovers from. `StressTest.setPool` and the POOL button (MIX → each of the five → MIX) give the species-alone runs; the pool's words go in CONDITIONS, and changing it clears any standing report, since a mixed run's numbers under a button reading QUEEN would be read as the queens'. **And the layout fix that had to come with it:** the report panel is `left:156 width:400` with `pointer-events:auto` and sits later in the DOM, so where it overlaps a tool button it does not merely cover it but EATS ITS TAPS. The right-hand column runs to y 156 at every supported size; the panel started at 96. With two buttons in the stress row that cost STOP a 22 px sliver, and POOL's third button cost it everything — the one control that abandons a seven-minute run, unpressable at Joshua's own 810 × 374. The panel moved to y 160, below the column, and the report now scrolls INSIDE it while COPY / RUN AGAIN / RESET stay pinned under it rather than parked below thirty lines. Verified in Chromium: every button pressable in every phase at 810 × 374 and 932 × 430. (A separate, pre-existing collision between the possess row and the tool row below ~700 px wide is NOT this change's and is not fixed here) | nothing |
| **6.13 The three-tier LOD ladder (alpha.44)** | **THE LOD CENTRE IS THE CAMERA, NEVER THE INSECT** (Joshua, 2026-09-10: "That range is based on the camera as that's what the player sees in 1st (not added yet), 3rd person, etc… needs to be around the camera not insect"). `FaunaView.update` takes an eye — a `WorldPoint` plus a height — and squares every creature's distance from it in 3D; the Lab reads that eye off `camera.position`, and a player build will read it off the player's viewing position, which is the same point in first or third person. Nothing anchors to the animal. **The ladder, from Joshua and ChatGPT's spec:** LOD0 inside 0.5 m is a full rig posed every frame; LOD1 from 0.5 to 0.8 m keeps the REAL mesh and silhouette with the bones held still and re-posed every `REDUCED_POSE_S` (250 ms); LOD2 past 0.8 m is the procedural impostor. The costly half of a rig is the per-frame bone work, not the draw, so LOD1 buys most of an impostor's saving while still looking like the animal — `pose()` takes a `refresh` flag and skips every `poseLegs`/`poseWings`/`poseAntennae`/`poseJaws`/`poseHead`/`poseGaster`/`layChain` call when it is false, and nothing else about the body changes. **Distance is PRIORITY; the budget is CAPACITY.** `allocate()` walks all species together nearest-first — one global queue, not a share per species — and hands out `fullBudgetFor(rung)` LOD0 slots and then `reducedBudgetFor(rung)` (`REDUCED_PER_FULL` = 2, a GUESS pending Baseline D) LOD1 slots. Being inside 0.5 m is a claim on a rig, never a guarantee of one: the radius-only rule it replaces made a crowd standing near the camera unaffordable, which is precisely the multiplayer case. **Every boundary is two numbers**, `LOD0_IN` 0.45 / `LOD0_OUT` 0.55 and `LOD1_IN` 0.75 / `LOD1_OUT` 0.85, held per creature id in `tier`, so a body sitting on a radius keeps the tier it has instead of alternating; crossings crossfade over `FADE_S` (0.2 s) with the rig's material opacity coming up as the impostor's instance-matrix SCALE goes down, and the fade — not the allocator — is what releases a holder, so a body leaving the ladder finishes fading before it loses its clone. (Reverting an alpha.43 "fix" that released on the mark and so deleted the fade-out; the test that justified it had read the count one frame after the crossing, mid-fade, and now settles for `FADE_S` first.) The pool is `full + reduced` clones — 117 clones / 22.7 ms at medium, a one-time load the test prints. **Still to do: LOD1's cheap material.** Joshua asked for a 64×64 texture (128×128 only if that reads badly) on the middle tier; alpha.44 ships only the frozen-bones half, which is the CPU half. **The Lab's bench sees none of LOD0** — its camera stands 1.00 m from the room's centre, so 0.0% of the floor is inside 0.45 m and 23.3% inside 0.85 m. That is the rule working on a 1 m room, not a bug, and the fix is to move the BENCH (four viewpoints priced in `docs/PERFORMANCE.md`), never the centre | LOD1's cheap texture; Baseline D prices `REDUCED_PER_FULL` |
| **6.14 The ladder's instrument, and the bench that could not see it (alpha.45)** | **Joshua, 2026-09-10, at 1,077 insects in the one-metre room: "LOD still not correct and rendering as a procedural too close… the room is a 1 meter block and from the camera center needs to be the center of the sphere… where do you have it?"** The centre was already the camera and is unchanged (6.13). What was wrong was that the BUDGET was binding and nothing said so: 13 full rigs and 26 frozen meshes is the whole of medium's capacity, so with hundreds of bodies inside `LOD0_IN` the fortieth-nearest drew as an ellipsoid at any distance, and the HUD printed `13` where it needed to print `13/13`. **A count with no cap beside it is not a reading** — it cannot tell "nothing nearer to draw" from "nothing left to give", which are opposite diagnoses with opposite fixes. `FaunaCost` therefore carries `fullBudget`/`reducedBudget` and the DEMAND (`withinFull`/`withinReduced`: who was inside each radius before any budget refused them), the live panel reads `drawn 13/13 rigs · 25/26 reduced` over `inside 412 at LOD0 · 998 at LOD1`, and the report's DRAWN block carries both. **Three defects found by going after it.** RANK was not hysteretic where distance was: with 400 bodies inside one radius and 13 rigs nobody crosses a boundary and the nearest thirteen are a different thirteen every frame, each swap a fade out and a fade in, so a perfectly stable crowd shimmers — a holder now sorts from `HOLD_ADVANTAGE` (0.8) of its distance and a challenger has to be a fifth nearer to displace it. `spare()` tested `rigged === 1`, filing a body wearing the FROZEN mesh with the ones fading out, so it could lose its clone to a claimant standing further away — a priority inversion in the one direction the ladder guarantees, latent while the pool equals the budget and live the moment either moves. And the lending loop walked `list[0 … fulls+reduceds]` rather than every mark: `wantsFull` reaches to `LOD0_OUT` for a body that already holds a rig and only to `LOD0_IN` for one that does not, so a holder a little further out can be marked after a nearer body is passed over, and the mark was spent with no mesh lent against it. **Two changes that move the numbers.** The pool was cut to the whole ladder's size for EVERY species up front — five pools' worth of skeletons for a frame that can only lend one pool's — and now warms to `POOL_WARM` and builds `POOL_GROWTH_PER_FRAME` more when an animal asks for one it cannot have, to the same per-species ceiling (a named pool is still cut exactly, because the Lab names one to say "rig these"); and RIGS cycles RUNG → ×2 → ×4 → ALL, because `fullBudgetFor` is the SUM OF `POOL_SIZES`, a clone table sized for an island where a handful of animals are near, and is not a measurement of anybody's phone — the multiples put the question on the button rather than in a number I would have guessed. **And the bench moved.** `FREE_START` stood 1.00 m from the middle of a 1 m room, so 0.0% of that floor was inside `LOD0_IN` and 23% inside `LOD1_OUT`: the ladder working exactly as specified on a bench that could never see it work. It now stands IN the room at 0.56 m — 23% of the floor at LOD0, 79% on a real mesh — against 77% of it framed rather than 96%, since you cannot stand inside a one-metre room and see all four corners through a 103° lens. Both columns measured against the real frustum at 932 × 430, tabulated in `docs/PERFORMANCE.md`. The answer to a bench with no LOD0 is always to move the BENCH | LOD1's 64×64 material; Baseline D prices `REDUCED_PER_FULL` |
| **6.15 The bench has no budget (alpha.46)** | **Joshua, 2026-09-11: "Remove any limits because it is a stress test, and if you keep adding rules, how can I actually get the correct numbers?"** He is right, and 6.14's RIGS ×2/×4 multiplier was the wrong shape of fix — a new rule layered over a limit that should never have been on a bench. **A bench that stops handing out rigs at thirteen measures thirteen**; the phone is never asked, the run flattens at the cap, and the frame rate it reports is about a bench that had stopped filling up. So `FaunaView.setUncapped(true)` is on for as long as the Creature Lab is open — no full-rig budget, no middle-tier budget, no pool ceiling (`fullBudget`, `reducedBudget` and `poolTarget` all answer `Infinity`, and the pool still grows a few clones a frame, so uncapped is affordable rather than preallocated) — and `setAllRigs` replaces the per-spawn `setPoolSize` dance that used to stand in for it. RIGS is two positions again and chooses only WHICH QUESTION: `ALL` rigs every drawn body whatever its distance (Baselines A and C), `LOD` lets the three radii decide and nothing else. The word RUNG went with the behaviour, because a button reading RUNG over a run with no rung budget in it is the lie the labels exist to stop. **This is the measurement that was wanted all along:** ALL prices a full rig, LOD from the same viewpoint prices what the frozen mesh and the impostor save, and from those three numbers any budget can be computed — including the one the GAME ships, which keeps its budgets. Measure uncapped, then choose a cap. **And the limits that are left cannot hide:** the report's `not drawn` splits into `underground` (a burrower below `BURROW_HIDE` — the renderer being right, and Baseline B's 55), `past a cap` and `cut far` (both LIMITS, labelled as such, zero on every run so far), so a ceiling can never again be counted alongside a worm. `MAX_CREATURES` (10,000) and `LAB_CAPACITY` (12,000 a species, which is also the impostor cap) stay, because the five impostor meshes preallocate instance matrices at that size; the ending already says when a ceiling rather than the frame rate stopped a run **And the bench measures at the PLAYER'S rung** (Joshua, same day: "should be on High to match settings not medium"): `CreatureLabHooks.settings` reads `settings.detail` through `detailFor`, the same call `PerformanceWorldScene` makes, and the rung is carried on the bench (`CreatureLab.rung`) so the simulation, the renderer and the report's CONDITIONS all take it from one place. A test or a probe with none to read gets `medium`. The line says what the rung DOES here, which uncapped is nothing — it sizes no budget and `LAB_SPECIES_TABLE` already flattens every population cap — because a condition that reads as a lever is the wrong kind of honest | raise the two ceilings if a run ever reaches one; LOD1's own geometry and material, which Baseline C-ALL says is where the frame actually is |
| **6.16 The ladder Joshua respecified (alpha.47)** | **"do full model and everything from 0-0.3m, from 0.3-0.5m, the texture will be a solid matching color but model still there and animated… from 0.5-0.6 it will fade to procedural and over 0.6m is procedural same solid color, no model or animation" (2026-09-11).** The middle tier CHANGED SIDES, and from his own measurement rather than from an opinion: it used to keep the textures and freeze the bones, and Baseline C-ALL priced that at 1.3 ms of a 31 ms frame for 198 skinned rigs. The frame is not the posing — it is the GPU submitting and shading those meshes — so the tier now gives up the TEXTURE and keeps the MOTION, which aims the saving at the fragment side where the other 29.6 ms is and keeps the thing that makes an ant read as alive. `skinsOf` builds BOTH coats when a clone is built and `skin()` only rebinds `mesh.material`: swapping a reference is free, swapping a material's `map` is a shader recompile and a hitch. The solid coat is `MeshLambertMaterial` in `LOOK[species].colour` — the impostor's own class and the impostor's own number, so "same solid color" is true by construction rather than by two tables agreeing — carrying `side` across, and carrying a CUTOUT (the original's `map` as an `alphaMap` at the same `alphaTest`) only where the original had one, since a fly's wing is a shaped alpha channel on a quad and would otherwise become cardboard. **And the crossfade became DISTANCE.** `meshShare(d²)` is 1 at `FADE_FROM`, 0 at `FADE_TO` and a straight ramp between, with the impostor drawing `1 - share`: no latch, no clock, nothing to start, hold, finish or get out of step with the tiers, which is what the timed fade it replaces kept doing. It needs no hysteresis because it is continuous; the one latching boundary left is the texture line (`TEXTURED_IN`/`TEXTURED_OUT`). A holder in the slack between `MESH_IN` and `MESH_OUT` is at zero share and is not submitted — the clone is the hysteresis, the draw call is not. `REDUCED_POSE_S`, the `refresh` gate through `pose`, and `FADE_S` all went with the old tier | run the same seed at RIGS: LOD against C-ALL's 31.0 ms; if the gap is small the answer is fewer triangles, not more tiers |
