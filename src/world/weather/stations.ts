/**
 * TWENTY-TWO PLACES ON KAUAʻI — the weather grid, carried from v0.
 *
 ...
 * and near-desert on the other: Waiʻaleʻale takes about 9,500 mm of
 * rain a year and Kekaha, twenty-five kilometres downwind, around 500.
 * Asking Līhuʻe airport what the weather is and painting the island
 * with the answer would throw away the most interesting fact about the
 * place. So: real coordinates, spread round the compass and up the
 * mountain, both ends of the gradient included, asked for in ONE
 * request. They are sample points, not spawns: the field interpolates,
 * so any place derives its weather from whichever stations are nearest.
 *
 * Coordinates are the real island's to about a hundred metres. They are
 * converted to world positions ONCE, here; the world position is what
 * everything downstream uses, because a station does not move when the
 * origin does.
 */
...
export const STATIONS: readonly Station[] = Object.freeze(SEEDS.map((seed) => {
  const where: GeoPoint = { lat: seed.lat, lon: seed.lon };
  return Object.freeze({ id: seed.id, name: seed.name, where, at: geoToWorld(where) });
}));

export const STATION_POINTS: readonly GeoPoint[] = Object.freeze(STATIONS.map((s) => s.where));
