 ...
 *
 * THIS IS v0.0.90's NUMBER, restored. v0.0.114 moved the exit down to
 * DRAUGHT on a real finding — the hydrology is live, soak sheds 0.3
 * units a second, and a pool that stops being fed crosses 3.4 mm
 * within a second of a shower ending, which puts her on the bed with
 * millimetres still over her. That finding stands and is worth
 * revisiting. It was not what Joshua was seeing: the same rule ran in
 * v0.0.90, which he tested and called good, so it cannot be the
 * regression. The audit found the disagreement in the DRAWING (see
 * IslandWater's FRESH_EDGE_LO), and tuning a gameplay threshold to
 * chase a rendering fault is how a build ends up with two wrong
