 ...
 *
 * The box is placed with MARGINS, so its parent must be a plain block
 * (not a grid or flex container that centres it again — a second centring
 * on top of the margin shifts the picture by a quarter of the overflow).
 *
 ...
 *   and its ratio and its caption are both somewhere else.
 * @returns a function that stops watching
 */
export function fitCover(box: HTMLElement, parent: HTMLElement, shape: () => Shape): () => void {
  const apply = (): void => {
  ...
  // ResizeObserver where it exists; the window's resize event where it
  // does not (jsdom, and nothing else this game runs on).
  ...
  if (typeof ResizeObserver !== 'undefined') {
    ...
    stop = () => view.removeEventListener('resize', apply);
  } else {
    stop = () => {};
  }
  apply();
